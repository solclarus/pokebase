...
type GoPokemonListItem = {
  pokemon_id: number;
  identifier: string;
  name: { ja: string; en: string };
  generation: number;
  forms: { form_id: string; released_at: string | null }[];
};

export function GoPokemonTable({ pokemons }: { pokemons: GoPokemonListItem[] }) {
      ...
      <Table>
        <TableHeader>
          <TableRow>
            ...
            <TableHead className="w-20 text-center">フォーム数</TableHead>
          </TableRow>
        </TableHeader>
        <TableBody>
          {pokemons.map((p) => (
            <TableRow
              ...
              onClick={() => router.push(`/pokemon/${p.pokemon_id}`)}
            >
